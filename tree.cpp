...
bool check_backward_overhang(IntervalNode* interval, uint exon_start, char read_strand, 
    ...
    std::string overhang_seq = extract_sequence(bundle->gseq, overhang_start, overhang_length, read_strand);
    for (const auto& tid : exon_tids) {
        
        // Use the TID-based edge to get the previous interval directly from current interval
        IntervalNode* prev_interval = g2t->getPrevNode(interval, tid, read_strand);
        
        if (prev_interval && prev_interval->end <= overhang_start + overhang_length) {
            ...
            std::string guide_seq = extract_sequence(bundle->gseq, prev_overhang_start, check_length, read_strand);
...
std::set<std::string> collapse_intervals(std::vector<IntervalNode*> sorted_intervals, uint exon_start, 
    ...
    if (!is_first_exon) {
        ...
        for (const auto& tid : first_interval->tids) {
            auto tid_last_interval = g2t->getPrevNode(first_interval, tid, read_strand);
    ...
    for (size_t i = 1; i < sorted_intervals.size(); ++i) {
        ...
        while (it != exon_tids.end()) {
            if (std::find(interval->tids.begin(), interval->tids.end(), *it) == interval->tids.end()) {
...
bool check_forward_overhang(IntervalNode* interval, uint exon_end, char read_strand,
    ...
    for (const auto& tid : exon_tids) {
        ...
        if (next_interval && next_interval->start >= overhang_start) {
            ...
            std::string guide_seq = extract_sequence(bundle->gseq, next_overhang_start, check_length, read_strand);
...
uint get_match_pos(IntervalNode* interval, std::string tid, char read_strand,
    ...
    uint prev_node_sum = g2t->getCumulativeLength(first_interval, tid, read_strand);
...
void process_mate_pairs(BundleData* bundle, std::map<std::string, ReadInfo*>& bam_info) {
    ...
    for (int i = 0; i < reads.Count(); i++) {
        ...
        for (int j = 0; j < read->pair_idx.Count(); j++) {
            ...
            if (mate_index < 0 || mate_index >= reads.Count() || !read_valid[mate_index]) {
                read_valid[i] = false;
            ...
            if (!common_transcripts.empty()) {
                ...
                // Case 2: Each mate maps to exactly one transcript, but different ones - allow it
                final_transcripts.insert(*read_transcripts.begin());
                final_transcripts.insert(*mate_transcripts.begin());
                
            } else {
                // Case 3: No common transcripts and at least one mate maps to multiple - skip this pair
                continue;
            }
            ...
            add_mate_info(final_transcripts, read_transcripts, mate_transcripts, read_positions, 
                mate_positions, bam_info, i, mate_index, read_size, mate_size);
        }
    }
}
