...
static GStr read_id("", 256);       // Prevent repeated reallocation for each parsed read
...
void process_paired_reads(BundleData& bundle, int bundle_start, int read_start,
    ...
    if (mate_start <= read_start) {
        std::string read_id = create_read_id(bundle.reads[read_index]->brec->name(), mate_start, hi);
